...
Final maxima were one cluster, one importer, one builder, and one task-owned PostgreSQL writer outside the dedicated session matrix. The concurrency-only peak was two recorded sessions. The cluster fast-stopped normally, the exact scratch root was removed, and `TASK_OWNED_RESIDUAL_PROCESS_COUNT=0`.
