#!/usr/bin/env python3
"""Capture Commons open-image records through publication category trees.
...
This is a source-count completion lane for the late release push. It allows
postage stamps as a publication/graphic communication object family while
keeping all records rights-aware, source-returnable, and subject to the
post-capture cleaning audit before public rebuild.
"""
...
RECORDS_CSV = DATA / "capture_batch_commons_open_publication_category_tree_2026_v1_records.csv"
SUMMARY_CSV = DATA / "capture_batch_commons_open_publication_category_tree_2026_v1_source_summary.csv"
QUALITY_CSV = DATA / "commons_open_publication_category_tree_2026_v1_quality.csv"
REPORT = DOCS / "COMMONS_OPEN_PUBLICATION_CATEGORY_TREE_2026_v1.md"
MANIFEST = CAPTURE_RUNS / "capture_run_manifest_v1.csv"
STATE_CSV = DATA / "commons_open_publication_category_tree_2026_v1_category_state.csv"
...
FILE_PAGE_LIMIT = 50
FILES_PER_CATEGORY_PAGES = 42
MAX_CATEGORY_VISITS = 12000
MAX_SUBCAT_DEPTH = 4
...
OBJECT_CAPS = {
    ...
    "book_cover": 780,
    "magazine_cover": 430,
    "advertising_trade": 740,
    "label_packaging": 720,
    "brochure_pamphlet": 620,
    "typography_identity": 500,
    "other": 220,
}

SEED_CATEGORIES = [
    ("postage stamp", "Category:Postage stamps by year"),
    ("postage stamp", "Category:Postage stamps by country"),
    ("advertisement", "Category:Advertisements by year"),
    ("advertisement", "Category:Advertisements by country"),
    ("book cover", "Category:Book covers by year"),
    ("poster", "Category:Posters by year"),
    ("film poster", "Category:Film posters by decade"),
    ("poster", "Category:Posters by year"),
    ("poster", "Category:Posters by country"),
    ("poster", "Category:Posters by topic"),
    ...
    ("advertising poster", "Category:Advertising posters"),
    ...
    ("label", "Category:Matchbox labels"),
    ("brochure", "Category:Brochures by country"),
    ("pamphlet", "Category:Pamphlets by country"),
    ("leaflet", "Category:Leaflets"),
    ("typography", "Category:Typography by country"),
    ("typography", "Category:Type specimens"),
    ("letterhead", "Category:Letterheads"),
    ("poster", "Category:Art school posters"),
    ("poster", "Category:University posters"),
    ("poster", "Category:Community posters"),
]

REVISIT_CATEGORIES = [
    ("postage stamp", "Category:1930s postage stamps"),
    ("postage stamp", "Category:1940s postage stamps"),
    ("postage stamp", "Category:1950s postage stamps"),
    ("postage stamp", "Category:1960s postage stamps"),
    ("postage stamp", "Category:1970s postage stamps"),
    ("postage stamp", "Category:1980s postage stamps"),
    ("postage stamp", "Category:1990s postage stamps"),
    ("postage stamp", "Category:2000s postage stamps"),
    ("postage stamp", "Category:2010s postage stamps"),
...
def expanded_infer_region_seeds() -> list[tuple[str, str, str, list[str]]]:
...
def object_from_category(default_object: str, category_title: str) -> str:
    ...
    if "label" in label:
        return "label"
    if "trade card" in label:
        return "trade card"
    if "advertisement" in label or "advertising" in label:
        return "advertisement"
    if "brochure" in label:
        return "brochure"
    if "pamphlet" in label:
        return "pamphlet"
    if "leaflet" in label or "flyer" in label:
        return "leaflet"
    if "letterhead" in label:
        return "letterhead"
...
def v2_object_family(row: dict[str, str], object_term: str) -> str:
    ...
    if "film poster" in blob or "movie poster" in blob or "cinema poster" in blob:
        return "film_poster"
    if "travel poster" in blob or "railway poster" in blob:
    ...
    if "label" in blob or "packaging" in blob or "package" in blob or "matchbox" in blob:
...
def v2_quality_gate(row: dict[str, str], object_term: str) -> tuple[bool, str]:
    blob = " ".join(
        [
    ...
    if v2_object_family(row, object_term) == "other":
        return False, "unclear_v2_object_family"
    return True, "ok"
...
def within_v2_caps(
    ...
    ok, reason = v3.within_distribution_caps(row, macro_counts, country_counts, period_counts, year_counts)
    if not ok:
        return ok, reason
    family = v2_object_family(row, object_term)
    if object_counts[family] >= OBJECT_CAPS.get(family, TARGET_ROWS):
        return False, "v2_object_family_cap"
    return True, "ok"


def should_capture_infer(category: str, object_term: str, depth: int) -> bool:
    label = category.lower()
    if depth > 3:
        return False
    if any(term in label for term in ("by country",)):
        return False
    return any(
        term in label
        for term in (
...
def update_manifest(records_count: int, active_sources: int, image_counts: Counter[str]) -> None:
    ...
    run_id = "commons_open_publication_category_tree_2026_v1"
    ...
    rows.append(
        {
            ...
            "notes": "Commons publication category-tree open image capture; source-derived metadata only; no image binaries or raw payloads saved.",
        }
    )
...
def write_outputs(
    ...
    *,
    ...
    for index, row in enumerate(rows, 1):
        row["capture_id"] = f"CPT2026R{index:05d}"
    ...
    summary_rows = [
        {
            ...
            "notes": "Commons publication category-tree open-license metadata; no image binary downloaded",
        }
    ...
    object_counts = Counter(v2_object_family(row, row.get("source_object_type", "")) for row in rows)
    ...
    lines = [
        "# Commons Open Publication Category Tree 2026 v1",
        "",
        f"Access date: {ACCESS_DATE}",
        "",
        "Scope: Commons publication category-tree discovery for open-license source pages. Metadata/source links/source-hosted image URLs only.",
        "",
        "## Metrics",
        "",
...
def main() -> None:
    base.REGION_SEEDS = expanded_infer_region_seeds()
    ...
    while queue and len(rows) < TARGET_ROWS and len(visited) < MAX_CATEGORY_VISITS:
        ...
        capture_allowed = bool(match) or should_capture_infer(category, object_term, depth)
        if capture_allowed and category not in completed:
            macro, country = match if match else ("_infer", "_infer")
            direction_country = country if match else "infer"
            offset: int | str = ""
            pages_seen = 0
            while pages_seen < FILES_PER_CATEGORY_PAGES and len(rows) < TARGET_ROWS:
                ...
                for page in sorted(pages, key=lambda item: item.get("index", 9999)):
                    row = base.row_from_page(
                        ...
                        macro,
                        country,
                        re.sub(r"[^a-z0-9]+", "_", f"category_tree_{direction_country}_{object_term}".lower()).strip("_"),
                        object_term,
                        url,
                    )
                    if not row:
                    ...
                    if not ok:
                        rejects[reason] += 1
                        continue
                    ok, reason = v2_quality_gate(row, object_term)
                    if not ok:
                        rejects[reason] += 1
                        continue
                    ...
                    if source_key in seen_ids or image_key in seen_images:
                        rejects["duplicate"] += 1
                        continue
                    cap_ok, cap_reason = within_v2_caps(
                        row,
                        object_term,
                        macro_counts,
                        country_counts,
                    ...
                    row["source_id"] = "SRC-COMMONS-PUBLICATION-CATEGORY-TREE-2026-V1"
                    row["source_object_type"] = f"Commons publication category-tree open image record; {object_term}"
                    row["classification_rationale"] = base.clean(
                        f"Selected by Commons publication category-tree category {category}, open-license extmetadata, strict source-derived graphic-object filter, duplicate exclusion, and object-year evidence.",
                        max_chars=700,
                    )
                    ...
                    object_counts[v2_object_family(row, object_term)] += 1
