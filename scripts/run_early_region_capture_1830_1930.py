...
RAW_DIR = DATA / "capture_batch_early_region_1830_1930_raw"
RECORDS_CSV = DATA / "capture_batch_early_region_1830_1930_records.csv"
SUMMARY_CSV = DATA / "capture_batch_early_region_1830_1930_source_summary.csv"
SURFACES_JSON = GENERATED / "public_surfaces_v1.json"
SOURCE_REGISTRY = DATA / "source_registry.csv"

ACCESS_DATE = "2026-05-30"
CAPTURE_BATCH_ID = "CB-EARLY-REGION-1830-1930"
USER_AGENT = "ModernGDHistory/0.1 early-region-capture"
YEAR_START = 1830
YEAR_END = 1930
TARGET_COUNT = 80
...
CAPTURE_PLAN = [
    {
        "direction_id": "ER01",
        "direction_name": "early_region_museum_poster_and_print_records",
    ...
    {
        "direction_id": "ER02",
        "direction_name": "early_region_design_museum_print_records",
        "source_name": "V&A Collections API",
        "adapter": "vam",
        "queries": ["trade card", "poster", "chromolithograph", "advertisement", "sheet music", "Art Nouveau poster"],
        "limit": 25,
    },
    {
        "direction_id": "ER03",
        "direction_name": "early_region_public_print_search_records",
        "source_name": "Library of Congress loc.gov API",
        "adapter": "loc",
        "queries": ["trade card", "advertising card", "chromolithograph poster", "sheet music cover", "poster"],
        "limit": 20,
    },
    {
        "direction_id": "ER04",
        "direction_name": "early_region_open_museum_print_records",
        "source_name": "Cleveland Museum Open Access API",
        "adapter": "cleveland",
        "queries": ["trade card", "poster", "chromolithograph", "advertisement"],
        "limit": 15,
    },
]
...
def in_scope(date_start: str, date_end: str, date_text: str) -> bool:
    ...
    if not (YEAR_START <= start <= YEAR_END):
        return False
    # Exclude broad century-level ranges such as 1800-1900. They can be useful
    # later as cards, but this visual-verification batch is anchored to the
    # early commercial-print and pre-/early-modern graphic design span.
    if end > YEAR_END and end - start > 20:
        return False
    return True
        ...
        summary_rows.append(
            {
                ...
                "notes": "Early region 1830-1930 production candidate batch; not final source record until review.",
            }
        )
...
def medium_for(row: dict[str, str]) -> tuple[str, str]:
    ...
    return folder_id("medium", "Commercial print"), "Commercial print"
...
def build_surface(row: dict[str, str], index: int) -> dict[str, Any]:
    ...
    theme_id = folder_id("theme", "Nineteenth-century commercial print ecology")
    theme_label = "Nineteenth-century commercial print ecology"
    folders = [
    ...
    surface = {
        ...
        "seqLabel": seq,
        "historicalNodeIds": ["HN004", "HN005"],
        "movementIds": [],
        "title": row.get("source_title") or f"Untitled early print record {index}",
        ...
        "tables": [
            ...
            table(
                ...
                [
                    ...
                    ("Historical node refs", "HN004; HN005"),
                    ("Classification basis", "Early-region 1830-1930 capture rule"),
                ],
            ),
            table(
                "RELATIONS",
    ...
    surface["_authorityRefs"] = {
        ...
        "mediaIds": [],
        "themeKeys": ["nineteenth-century-commercial-print-ecology"],
...
def scope_note(folder_type: str, title: str) -> str:
    if folder_type == "region":
        return f"Early-region records from 1830-1930 associated with {title}."
    if folder_type == "theme":
        return "Early modern commercial print, advertising, chromolithography, trade-card, catalogue, and public visual circulation records from 1830-1930."
    if folder_type == "medium":
        return f"Early-region 1830-1930 records filed by medium: {title}."
...
def build_public_payload(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "meta": {
            ...
            "status": "generated",
            "note": "Generated early-region 1830-1930 visual-verification payload. Static export; not final publication data.",
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Capture and generate early-region public surfaces.")
    parser.add_argument(
        "--from-csv",
        action="store_true",
    ...
    for index, row in enumerate(rows, start=1):
        row["capture_id"] = f"ER1830R{index:03d}"
