...
Architecture PASS requires an exact-research-pair-bound async contract, three adapters (`Http`, immutable release, fixture), stable DTOs, keyset pagination, typed results/errors, exact `(researchReleaseId,researchManifestSha256)` plus an atomic optional `(visualRegistryVersion,visualRegistrySha256)` pair, declared compatibility, normal research-only behavior when the registry is absent, explicit mismatch without fallback, schema validation, cache/deduplication/cancellation responsibilities, and GET-only pair-pinned endpoints.
...
- candidate closure fixes snapshot/cohort/corpus/query/registry/claim/projection/asset fingerprints for research, or reference/bridge/provider/locator/rights-assessment/policy-evaluation/delivery/health/takedown/asset fingerprints for visual;
...
Research-current may advance independently into an explicit visual-mismatch window. In that window research remains resolvable, every visual locator is absent, and no older visual registry is inherited. Visual-current CAS must validate its target against the guarded expected research-current generation/pair; a stale CAS fails without writing either pointer.
...
Pre-DDL decision PASS requires stable domain-independent object/relation/claim/source/visual-reference URNs; deterministic future HTTPS resolver mapping without `.example` placeholders; exact research and optional visual response identity; GET/HEAD/OPTIONS-only public semantics; closed `SAFE`/`PUBLIC`/`INTERNAL`/`HELD` field classes; a positive serializer allowlist; explicit registry-absent/mismatch behavior; and structural non-disclosure of held/raw/internal or non-`REMOTE_IMAGE` pixel locators.

Implementation PASS later requires an actual Read API, OpenAPI and JSON Schemas; server-rendered crawlable HTML/canonical HTTPS aliases; JSON-LD with Linked Art/PROV-O mappings; DCAT; content negotiation/link headers; URI persistence/redirect tests; diff/change feed; sitemap/robots; crawlability, determinism and rights-leakage tests. Documentation-only design is PASS for the pre-DDL decision gate but PARTIAL for implementation. A write endpoint in `/api/v1`, unstable relation/claim identity, client-only canonical content, or held-pixel leakage is FAIL.

## G15 — Rights-aware visual federation

Pre-DDL decision PASS requires an independent sealed visual registry with public `(visualRegistryVersion,visualRegistrySha256)`, its own manifest/sidecar/current-pointer CAS, exactly one declared compatible research pair, provenance-occurrence visual-reference identity, the N:M object-reference bridge, typed provider objects/locators, and closed real-FK observation/assessment/takedown targets. Rights observations/assessments, provider-policy versions/evaluations, delivery decisions, endpoint-health observations and takedown state are independent records; attribution/required statements and review due are explicit evidence. The legacy baseline must prove 100% inventory, 100% typed disposition and zero unclassified visual references; positive-rights coverage is measured but is not a PASS threshold.

Implementation PASS later requires the closed modes `BLOCKED`, `CITATION_ONLY`, `LINK_ONLY`, `SOURCE_VIEWER` and `REMOTE_IMAGE`; default link/citation for unknown/missing/conflicting/stale rights or policy; zero widening from endpoint accessibility or IIIF availability; structural omission of lower-mode pixel/thumbnail/image-service fields; verified attribution; active-takedown precedence; and seal/CAS/non-disclosure tests. Documentation and the typed v48 baseline are PASS for the pre-DDL decision gate but PARTIAL for implementation. Treating HTTP success, API access, IIIF, redirect, or provider ownership as image authorization is FAIL.

## Promotion rule

Pre-DDL completion may be accepted when G0–G3, the decision portions of G4–G8 and G12–G15, G9, and G11 pass while implementation portions and G10 remain PARTIAL. The Phase 1C and Phase 1D gate receipts—not the Phase 1B audit alone—must be independently verified. `ENGINEERING_PRE_DDL_READY`, `RESEARCH_SEMANTICS_PRE_DDL_READY`, `RIGHTS_VISUAL_PRE_DDL_READY`, `MACHINE_CONTRACT_PRE_DDL_READY`, and `OVERALL_PRE_DDL_READY` are evaluated separately. Missing API/OpenAPI/JSON Schema/JSON-LD/CI/deployment/browser implementation does not by itself keep physical-schema specification blocked; it does keep freeze, frontend promotion and deployment false.
