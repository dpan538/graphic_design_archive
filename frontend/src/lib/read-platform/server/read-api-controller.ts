...
function failure<T>(result: Extract<RepoResult<T>, { ok: false }>, instance: string, version?: ArchiveVersionRef, head = false) {
  const httpStatus = status(result.error.code);
  return head
    ? new NextResponse(null, { status: httpStatus, headers: readApiHeaders(version) })
    : NextResponse.json({ type: `urn:gdarchive:problem:${result.error.code.toLowerCase().replaceAll("_", "-")}`, title: "Read API request failed", status: httpStatus, code: result.error.code, detail: result.error.message, instance }, { status: httpStatus, headers: readApiHeaders(version) });
}
...
export async function dispatchReadApiRequest(request: Request, path: readonly string[], provider: ArchiveRepositoryProvider = getArchiveRepositoryProvider()): Promise<Response> {
  ...
  try {
    ...
    const result = await resource(repo, tail, url, request.signal);
    const failureInstance = !result.ok
      && result.error.code === "NOT_FOUND"
      && isContextResourcePath(tail)
      ? "/api/v1/releases/{release}/trace/objects/{id}/context"
      : url.pathname;
    return result.ok ? success(result, head) : failure(result, failureInstance, repo.version, head);
  } catch (error) {
...
async function resource(repo: ArchiveRepository, tail: readonly string[], url: URL, signal?: AbortSignal) {
  ...
  if (isContextResourcePath(tail)) {
    return repo.getTraceContext
      ? repo.getTraceContext(tail[2], { signal })
      : { ok: false as const, error: { code: "NOT_FOUND" as const, message: "Context dataset is not available for this object.", retryable: false } };
  }
