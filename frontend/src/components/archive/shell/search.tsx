...
/** Compact entry point only. Results come from the public object Search API on explicit submit. */
  ...
  return (
    <div id="archive-search-panel" className="corner-card search-card flex flex-col" role="search">
      ...
      <form action="/search" method="get">
        <label className="sr-only" htmlFor="shell-search-query">Search public objects by ID, title, credited name, or place</label>
        <input
          id="shell-search-query"
          name="q"
          type="search"
          placeholder="ID, title, name, or place"
      ...
      <p className="text-ink-soft mt-2" style={{ fontSize: "0.6rem" }}>
        Deterministic matching supports public object metadata and four hard filters.
      </p>
