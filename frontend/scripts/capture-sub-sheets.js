...
const GROUP_SELECTOR = '[data-sub-sheet-group="group-01"]';
...
async function auditPage(page) {
  return page.evaluate((selector) => {
    ...
    if (pages.length !== 4) issues.push(`Expected 4 sub sheets, found ${pages.length}`);
...
async function main() {
  ...
  try {
    ...
    if (group) {
      const groupPath = path.join(runDir, "sub-sheets-group-01.png");
